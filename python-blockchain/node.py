...
@app.route('/broadcast-transacion', methods=['POST'])
...
def broadcast_block():
    ...
    if block['index'] == blockchain.chain[-1].index + 1:
        ...
        response = {'message': 'Blockchain seems to differ from other blocks'}
...
def mine():
    if blockchain.resolve_conflicts == True:
        response = {'message': 'Resolve conflicts first, blocks not added'}
        return jsonify(response), 409
